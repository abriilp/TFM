...
import dino_vits as vits
nn = torch.nn
F = torch.nn.functional
class DinoFeaturizer(nn.Module):

    def __init__(self, patch_size = 8, arch = 'vit_small'):
        super().__init__()
        #self.dim = dim
    ...
    def forward(self, img):
        ...
        with torch.no_grad():
            ...
            feat = self.model.forward_feats(img)
            #feat, attn, qkv = feat[0], attn[0], qkv[0]

            feat_h = img.shape[2] // self.patch_size
            feat_w = img.shape[3] // self.patch_size
            return feat[:, 1:, :].reshape(img.shape[0], feat_h, feat_w, -1).permute(0,3,1,2)

#model = DinoFeaturizer()
#model.cuda()
#feat = model(torch.randn(10, 3, 256, 256).float().cuda())
#pdb.set_trace()
