...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.proj_light = nn.Linear(2, num_heads, bias=bias)


    def forward(self, x, des_light):
        b,c,h,w = x.shape

        feat_light = des_light.view(-1, 2, 1, 1).expand(-1, -1, h, w)
        light_prompt = self.conv_light(feat_light) #.permute(0, 2, 3, 1).view(-1, h*w, self.light_dim)

        x = x + light_prompt

        # print(x.shape, light_prompt.shape) # 1, 48, 256, 256
        # exit(0)
        x = self.qkv(x)
        # print(x.shape) # 1, 144, 256, 256
        qkv = self.qkv_dwconv(x)
        # print(qkv.shape) # 1, 144, 256, 256
        ...
        # print(q.shape, k.shape, v.shape) # [1, 1, 48, 65536] [1, 1, 48, 65536] [1, 1, 48, 65536]
        # exit(0)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, des_light):
        x = x + self.attn(self.norm1(x), des_light)
...
class Restormer(nn.Module):
    ...
    def forward(self, inp_img, des_light):
        ...
        for blk in self.decoder_level3:
            out_dec_level3 = blk(out_dec_level3, des_light)
        ...
        for blk in self.decoder_level2:
            out_dec_level2 = blk(out_dec_level2, des_light)

        inp_dec_level1 = self.up2_1(out_dec_level2)
        inp_dec_level1 = torch.cat([inp_dec_level1, out_enc_level1], 1)
        out_dec_level1 = inp_dec_level1
        for blk in self.decoder_level1:
            out_dec_level1 = blk(out_dec_level1, des_light)
