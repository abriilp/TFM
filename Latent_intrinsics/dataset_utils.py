...
class RLSIDDataset(Dataset):
    def __init__(self, root_dir, img_transform=None, validation_scenes=None, 
        ...
        self.normals_dir = '/home/apinyol/TFM/Data/RLSID/Image/normals'
        if not os.path.exists(self.normals_dir):
            raise ValueError(f"Normals directory not found: {self.normals_dir}")
        ...
        if not self.is_validation and self.max_training_images is not None:
            if self.max_training_images < len(self.all_images):
                # Shuffle for random sampling, but set seed for reproducibility
                import random
        ...
        # Pre-compute valid scene pairs for validation AFTER all_images is created
        if self.is_validation:
    ...
    def _load_normals(self, normals_path):
        ...
        try:
            ...
            #normals_img = transform(normals_img)
    ...
    def __getitem__(self, idx):
        ...
        if self.is_validation:
            #print(f"Validation mode: Fetching item {idx} from dataset")
            # VALIDATION MODE: 3 images + 2 normals
            # 1. Input: from validation scene
            # 2. Reference: from different validation scene, any lighting that exists in input scene
            # 3. GT: same scene as input, same lighting as reference
            # 4. Input normals: corresponding to input image
            # 5. Reference normals: corresponding to reference image
            ...
            # Choose a random reference scene
            ref_scene = random.choice(valid_ref_scenes)
            ...
            # Choose a random lighting condition from the available ones
            ref_lighting = random.choice(available_ref_lightings)
            ...
            # Load normals for input and reference images
            ...
            #print(f"Training mode: Fetching item {idx} from dataset")
            # TRAINING MODE: 2 images from same scene, different lighting
            ref_candidates = []
            ...
            if input_scene_bg in self.image_data:
                ...
                if available_lightings:
                    ref_lighting = random.choice(available_lightings)
            ...
            if ref_candidates:
                ...
                if len(scene_images) > 1:
                    ...
                    if ref_candidates:
                        ref_info = random.choice(ref_candidates)
                        ref_path = ref_info['path']
                    else:
                        ref_path = input_path
                else:
            ...
            # Load input normals (only input normals for training mode)
            input_normals = self._load_normals(input_normals_path)
            #print(f"input normals type: {type(input_normals)}")
...
# Example usage:
"""
# Specify validation scenes, training uses all others
train_dataset = RLSIDDataset(
    root_dir='/path/to/dataset',
    validation_scenes=['004', '005'],    # Validation uses scenes 004, 005
    is_validation=False,                 # Training uses all other scenes automatically
    img_transform=[group_transform, single_transform]
)

val_dataset = RLSIDDataset(
    root_dir='/path/to/dataset',
    validation_scenes=['004', '005'],
    is_validation=True,
    img_transform=[group_transform, single_transform]
)

# Auto-split (if you don't specify validation_scenes)
auto_train_dataset = RLSIDDataset(
    root_dir='/path/to/dataset',
    is_validation=False,
    img_transform=[group_transform, single_transform]
)
